            ...
            <motion.article key={cs.id} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="brand-surface overflow-hidden">
              <div className={`grid lg:grid-cols-2 ${i % 2 === 1 ? "lg:[direction:rtl]" : ""}`}>
                ...
                <div className={`p-8 lg:p-10 ${i % 2 === 1 ? "lg:[direction:ltr]" : ""}`}>
                  ...
                  <p className="mt-2 text-sm text-[var(--glitz-gold)]">Investment: {cs.budget}</p>
                  <div className="mt-4 space-y-2 text-sm"><p><strong>Challenge:</strong> <span className="text-[var(--glitz-muted)]">{cs.challenge}</span></p><p><strong>Solution:</strong> <span className="text-[var(--glitz-muted)]">{cs.solution}</span></p><p><strong>Result:</strong> <span className="text-[var(--glitz-muted)]">{cs.result}</span></p></div>
