    ...
    <div className="brand-root">
      ...
      <BrandSection>
        <div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-20">
          <div>
            ...
            <p className="mt-4 text-muted leading-relaxed">Every celebration carries the weight of memory. That responsibility drives everything we do at Glitz Events & Promotions.</p>
      ...
      <BrandSection alt>
        <div className="grid gap-6 md:grid-cols-3">
          {[{ icon: Eye, t: "Vision", c: companyProfile.vision }, { icon: Target, t: "Mission", c: companyProfile.mission }, { icon: Gem, t: "Philosophy", c: "Luxury is intention — every detail deliberately crafted." }].map(({ icon: Icon, t, c }) => (
            <motion.div key={t} whileHover={{ y: -4 }} className="brand-surface p-8"><Icon className="mb-4 h-8 w-8 text-[var(--glitz-gold)]" /><h3 className="brand-display text-xl font-semibold">{t}</h3><p className="mt-3 text-sm text-muted">{c}</p></motion.div>
          ))}
        </div>
      </BrandSection>
      <BrandSection>
      ...
      <BrandSection alt>
        ...
        <div className="grid gap-4 sm:grid-cols-2">{BRAND_AWARDS.map((a) => (
          <div key={a.title} className="brand-surface flex gap-4 p-6"><Award className="h-8 w-8 shrink-0 text-[var(--glitz-gold)]" /><div><h3 className="font-semibold">{a.title}</h3><p className="text-sm text-muted">{a.org} · {a.year}</p></div></div>
        ))}</div>
        <div className="mt-10 flex flex-wrap justify-center gap-3">{BRAND_MEDIA.map((m) => <span key={m} className="rounded-full border border-[var(--glitz-border)] px-5 py-2 text-sm text-muted">{m}</span>)}</div>
      </BrandSection>
    </div>
  );
}
