...
export function BrandHeader({ label, title, subtitle, center }: { label?: string; title: string; subtitle?: string; center?: boolean }) {
  return (
    <header className={cn("mb-12 sm:mb-16", center && "mx-auto max-w-3xl text-center")}>
      ...
      <h2 className="brand-display text-[clamp(1.75rem,4vw,3rem)] font-semibold leading-tight text-primary">{title}</h2>
      {subtitle && <p className="mt-4 text-base leading-relaxed text-secondary sm:text-lg">{subtitle}</p>}
    </header>
  );
}
